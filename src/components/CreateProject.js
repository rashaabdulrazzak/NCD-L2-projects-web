...
import { Form, Button ,Row,Col,FloatingLabel,InputGroup,Toast, Container} from 'react-bootstrap';
import { connect, keyStores } from 'near-api-js'


const CreateProject = ({ contract, currentUser, nearConfig, wallet }) => {
...
const keyStore = new keyStores.BrowserLocalStorageKeyStore();
  ...
  return (
    <Container>
      <Row className="my-5">
        <Col md={{ span: 8, offset: 2 }} className="d-flex justify-content-center secondary-color">
          <h2> Submit a project to get funds</h2>
        </Col>
       </Row>    
      <Row>
        <Col md={{ span: 8, offset: 2 }}>      
        { showA == true ? <Toast show={showA} onClose={toggleShowA} position="top-end" bg="success">
                  <Toast.Header>           
                    <strong className="me-auto">Congrat</strong>            
                  </Toast.Header>
                  <Toast.Body>Your Project has been successfully created</Toast.Body>
          </Toast> : 
          <Toast show={showB} onClose={toggleShowB} position="top-end" bg="danger">
                  <Toast.Header>           
                    <strong className="me-auto">Failed</strong>            
                  </Toast.Header>
                  <Toast.Body>Your Project Address should be an existing Address on the testnet</Toast.Body>
          </Toast> 
        }
      <Form onSubmit={handleSubmit} className="mt-3 pt-3">
        ...
        />
        <br/>
        <FloatingLabel controlId="floatingSelect" label="Select a Category of your project">
        <Form.Select
        defaultValue={selectedCat}
        onChange={({target})=>{setSelectedCat(target.value)}}
      ...
      videoUrl   https://example.com/users/
      </InputGroup.Text>
